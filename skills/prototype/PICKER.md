...
Verbatim spec, adapted from emilkowalski/skills `prototype/PICKER.md`. The only values that change per run are variant names and count.
...
- Selection persists across reload via a URL param (`?variant=<name>`, the same param the switcher reads), falling back to variant 1. The highlight takes its initial position without animating (`data-ready` is added after first paint).
...
```js
// `variants` is an array of render functions and `names` their slugs, in picker order.
...
function setActive(i) {
  ...
  url.searchParams.set('variant', names[i]);
...
setActive(Math.max(0, names.indexOf(new URLSearchParams(location.search).get('variant'))));
