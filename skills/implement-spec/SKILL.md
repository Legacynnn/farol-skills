...
4. **Dispatch the frontier.** For each unblocked, unclaimed ticket, claim it in the tracker and start an **implementer subagent** in the background, in its own worktree on `feature/<spec-slug>/<ticket>`, with this brief and nothing more:
   ...
   > Implement ticket <id> at <path or url>. Read `docs/agents/README.md` first. Call the Skill tool with "tdd" at the seams the ticket names. For any screen, call the Skill tool with "finish-screen". Commit on your branch with the ticket id in the message. Report: commits, criteria met, criteria not met and why, anything the next ticket must know. Under 200 words.
