...
4. Run the passes in order. Apply the `mobile-first` rules (its SKILL.md sits beside this one) from 320px up. Call the Skill tool twice, for "better-ui" and "emil-design-eng", and fix polish and motion against both. Apply the `fix-copy` rules to every string.
