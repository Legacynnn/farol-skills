...
2. **Pick the home**: this project (`.claude/skills/<name>/`, for a skill that only makes sense here, like `verify`) or farol-skills (`experimental/<name>/` first; it moves to `skills/` once it has run for real). Done when the folder exists.
...
6. **Register**: in farol-skills, add a row to the README skills table (`experimental` status), and re-run `scripts/link-skills.sh`; on promotion to `skills/`, add it to `.claude-plugin/plugin.json`, `ask-farol`, bump the version; in a project, add a line to `docs/agents/README.md` saying when to reach for it. Done when `claude plugin validate . --strict` passes (farol) or the pointer line exists (project).
